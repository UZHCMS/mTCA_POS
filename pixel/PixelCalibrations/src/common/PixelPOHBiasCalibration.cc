...
void PixelPOHBiasCalibration::beginCalibration() {
  ...
  if (tempCalibObject->parameterValue("POHGain")      != "") POHGain         = atoi(tempCalibObject->parameterValue("POHGain").c_str());//gain values for which the scan is performed
  ...
  for (std::vector<unsigned>::const_iterator gain_itr = POHGains.begin(); gain_itr != POHGains.end(); ++gain_itr) {
    cout << "POH gain itr " << itoa(*gain_itr) << endl;
...
bool PixelPOHBiasCalibration::execute() { 
  ...
  for (std::vector<unsigned>::const_iterator gain_itr = POHGains.begin(); gain_itr != POHGains.end(); ++gain_itr) {
    cout << "execute POH gain itr " << itoa(*gain_itr) << endl;
