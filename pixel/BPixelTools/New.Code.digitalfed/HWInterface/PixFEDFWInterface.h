...
class PixFEDFWInterface : public RegManager
{
  ...
  private:
    ...
    uint32_t fNCh ;          // the number of Channels per TBM = 2
    uint32_t fNPattern ;     // the number of Patterns to send per channel = 8 (8 ROCs per TBM channel)
    uint32_t fPacketSize ;   // the size of a packet for each acquisition = 2;
