...
void RegReset() {
  ...
  VMEout(0x3,LAD_C+0x1a0000,4,0x1001c); // Try this******************* 
...
void SendTriggerAndReadFifo1(bool debug = true) {
  ...
  for(int j1=0;j1<256;j1++){ 
    ...
    if (fedchannel<1 || fedchannel>36)
      return; 
    if (fedchannel<10) {
      VMEin(0x3,LAD_N+CH_SubAddr[fedchannel-1],4,&d);
      printf ("fedchannel %d",fedchannel);
      printf("Fifo-1 N %d %x\n ",j1,d);
    } 
    else if (fedchannel<19) {
      VMEin(0x3,LAD_NC+CH_SubAddr[fedchannel-10],4,&d);
      printf ("fedchannel %d",fedchannel);
      printf("Fifo-1 NC %d %x\n ",j1,d);
    } 
    else if (fedchannel<28) {
      VMEin(0x3,LAD_SC+CH_SubAddr[fedchannel-19],4,&d);
      printf ("fedchannel %d",fedchannel);
      printf("Fifo-1 SC %d %x\n ",j1,d);
    } 
    else if (fedchannel<36) {
      VMEin(0x3,LAD_S+CH_SubAddr[fedchannel-28],4,&d);
      printf ("fedchannel %d",fedchannel);
      printf("Fifo-1 S %d %x\n ",j1,d);
    }
    else
...
int main(int argc, char *argv[]) {
  ...
  //Other settings
  //VMEout (0x3,LAD_C+0x1a0000,4,0x10014);   //set the gdmf control register
  VMEout(0x3,LAD_C+0x1a0000,4,0x1001c); // 0-TTCEn-EventInt-EnableDAC-0-0//Try this************************
  VMEout (0x3,LAD_C+0x1d8000,4,0x0); // Trigger delay in the center chip
