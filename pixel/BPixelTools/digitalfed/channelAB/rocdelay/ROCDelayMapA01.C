{ // ROOT scripts always need set of {} to do all the script
	...
	g->SetTitle("ErrorRateMap channel A b6_0 b7_1");
	g->SetStats(false);
	
	g->Draw("colz");
	
	string plotFileName = + fileName + "_ErrorRateMap_channel_A_b6_0_b7_1.pdf";
	c1->SaveAs(plotFileName.c_str());
	plotFileName = fileName + "_ErrorRateMap_channel_A_b6_0_b7_1.png";
	c1->SaveAs(plotFileName.c_str());	
