{ // ROOT scripts always need set of {} to do all the script
	...
	g->SetTitle("ErrorRateMap channel B b6_0 b7_0");
	g->SetStats(false);
	
	g->Draw("colz");
	
	string plotFileName = + fileName + "_ErrorRateMap_channel_B_b6_0_b7_0.pdf";
	c1->SaveAs(plotFileName.c_str());
	plotFileName = fileName + "_ErrorRateMap_channel_B_b6_0_b7_0.png";
	c1->SaveAs(plotFileName.c_str());	
