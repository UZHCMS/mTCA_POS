...
void TitleCalInject (int q) {
  cout << "#Sent triggers: " << q*burst << "\tand with a frequency [Hz] of: " << ((float)4/(float)rate*10000000) << endl;
  cout << "#RocNr" << "\tColAddr" << "\tRowAddr" << "\tPixelHight"  << endl;
}
...
{                     
  ...
  for (int p = 0; p < q; p++)        // will go through the data for each burst in the total number of events q
	...
	{    
	  ...
	  while((!isEmptyA || !isEmptyB) && !(cnt > 6))
	      ...
	      if (canreadoutA = true)              // write input of cSpyA in cSpyA_Type
		{
		  for (int i=0; i < cSpyA.size(); i++){
		    if ( (cSpyA[i] & 0xff) != 0) {
		      //cout << hex << ((cSpyA[i] >> 4) & 0xff) << " " ;
		      cSpyA_Type.push_back(((cSpyA[i]) & 0xff));}                       // cSpyA_Type should contain just "8 9 a b 7 7 7 7 7 7 7 7 c d e f ..." so only important information of tbm header/trailer and ROC header
		  }
		}
	      else 
		cout << "cSpyA is empty" << endl;  
		...
		{             
		  for (int i=0; i < cSpyB.size(); i++){
		    if ( (cSpyB[i] & 0xff) != 0) {
		      ...
		      cSpyB_Type.push_back(((cSpyB[i]) & 0xff));}                        // cSpyB_Type should contain just "8 9 a b 7 7 7 7 7 7 7 7 c d e f ..." so only important information of tbm header/trailer and ROC header
		  }
		}
	      else 
		cout << "cSpyB is empty" << endl; 
	  ...
	  //cout << dec << "sizeA" << cSpyA_Type.size() << endl;	  
	    //for (int a=0; a < cSpyA_Type.size(); a++){
	    //cout << hex  << cSpyA_Type[a]  << "\t";}
	  //cout << dec << endl;
	  ...
	  while ((iB < size) || (iA < size)){ ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	    ...
	    // analize channel A
	    if(( ((iA+4) < sizeA) && (cSpyA_Type[iA]& 0xf0) == 0x80) && ((cSpyA_Type[iA+1]& 0xf0) == 0x90) && ((cSpyA_Type[iA+2]& 0xf0) == 0xa0) && ((cSpyA_Type[iA+3]& 0xf0) == 0xb0) ){        // if we find a tbm header
	      iA += 4;                                                                                                                                                     // we look at the things behinde the header
	      overflowA = 0;
	      //rocheaderA = false;
	      // cout << "2" << endl;
	      while ( ((iA+1) < sizeA) && ((cSpyA_Type[iA]& 0xf0) != 0x80) && ((cSpyA_Type[iA]& 0xf0) != 0x90) && ((cSpyA_Type[iA]& 0xf0) != 0xa0) && ((cSpyA_Type[iA]& 0xf0) != 0xb0) &&    // till we see (part of) tbm header or trailer
		                          ((cSpyA_Type[iA]& 0xf0) != 0xc0) && ((cSpyA_Type[iA]& 0xf0) != 0xd0) && ((cSpyA_Type[iA]& 0xf0) != 0xe0) && ((cSpyA_Type[iA]& 0xf0) != 0xf0)){
		overflowA++;
		//	cout << "2.5" << endl;
		rocheaderA = false;
		colAddrA = 0;
		...
		if ((cSpyA_Type[iA]& 0xf0) == 0x70){
		  rocheaderA = true;
		  overflowA = 0;
		  //rocA++;                                                                  // count roc header till we see (part of) tbm header or trailer
		  while (((iA+1) < sizeA) && (rocheaderA == true)  && ((cSpyA_Type[iA]& 0xf0) != 0x80) && ((cSpyA_Type[iA]& 0xf0) != 0x90) && ((cSpyA_Type[iA]& 0xf0) != 0xa0) &&  // till we see (part of) tbm header or trailer
		                     	     ((cSpyA_Type[iA]& 0xf0) != 0xb0) && ((cSpyA_Type[iA]& 0xf0) != 0xc0) && ((cSpyA_Type[iA]& 0xf0) != 0xd0) && ((cSpyA_Type[iA]& 0xf0) != 0xe0) && ((cSpyA_Type[iA]& 0xf0) != 0xf0) ){
		   if ((cSpyA_Type[iA]& 0xf0) == 0x70){
		     rocA++;
		     //	     cout << "rocA: " << rocA << endl;
		   }
		    if (((iA+6) < sizeA)  && ((cSpyA_Type[iA+1]& 0xf0) == 0x10) && ((cSpyA_Type[iA+2]& 0xf0) == 0x20) && ((cSpyA_Type[iA+3]& 0xf0) == 0x30) && 
		                                                   ((cSpyA_Type[iA+4]& 0xf0) == 0x40) && ((cSpyA_Type[iA+5]& 0xf0) == 0x50) && ((cSpyA_Type[iA+6]& 0xf0) == 0x60) ) {
		      ...
		      word = ((cSpyA_Type[iA+1] & 0xf) << 20) + ((cSpyA_Type[iA+2]& 0xf) << 16) + ((cSpyA_Type[iA+3]& 0xf) << 12) + ((cSpyA_Type[iA+4]& 0xf) << 8) + ((cSpyA_Type[iA+5]& 0xf) << 4) + (cSpyA_Type[iA+6]& 0xf);

		      pixelHitA = (word & 0x0f) + ((word >> 1) & 0xf0);
		      ...
		      // cout << dec << "Hit: " << rocA << "\t" << colAddrA << "\t" << rowAddrA << "\t" << pixelHitA << endl;
		      colAddrA = 0;
		      rowAddrA = 0;
		      pixelHitA = 0;
		...
		if (((cSpyA_Type[iA]& 0xf0) == 0x70) || ((cSpyA_Type[iA]& 0xf0) == 0x80) || ((cSpyA_Type[iA]& 0xf0) == 0x90) || ((cSpyA_Type[iA]& 0xf0) == 0xa0) ||  // till we see (part of) tbm header or trailer
		    ((cSpyA_Type[iA]& 0xf0) == 0xb0) || ((cSpyA_Type[iA]& 0xf0) == 0xc0) || ((cSpyA_Type[iA]& 0xf0) == 0xd0) || ((cSpyA_Type[iA]& 0xf0) == 0xe0) || ((cSpyA_Type[iA]& 0xf0) == 0xf0) ){
		  iA--;	}
		...
		rocA = 0;}
	      ...
	      if ( (iA < (sizeA + 4)) && ((cSpyA_Type[iA]& 0xf0) == 0xc0) && ((cSpyA_Type[iA+1]& 0xf0) == 0xd0) && ((cSpyA_Type[iA+2]& 0xf0) == 0xe0) && ((cSpyA_Type[iA+3]& 0xf0) == 0xf0) )     // if after ROC header tbm trailer: skip this tbm header
		iA += 4;
	    }
	    ...
	    // analize channel B
	    if(( ((iB+4) < sizeB) && (cSpyB_Type[iB]& 0xf0) == 0x80) && ((cSpyB_Type[iB+1]& 0xf0) == 0x90) && ((cSpyB_Type[iB+2]& 0xf0) == 0xa0) && ((cSpyB_Type[iB+3]& 0xf0) == 0xb0) ){        // if we find a tbm header
	      iB += 4;                                                                                                                                                     // we look at the things behinde the header
	      overflowB = 0;
	      //rocheaderB = false;
	      // cout << "2" << endl;
	      while ( ((iB+1) < sizeB) && ((cSpyB_Type[iB]& 0xf0) != 0x80) && ((cSpyB_Type[iB]& 0xf0) != 0x90) && ((cSpyB_Type[iB]& 0xf0) != 0xa0) && ((cSpyB_Type[iB]& 0xf0) != 0xb0) &&    // till we see (part of) tbm header or trailer
		                          ((cSpyB_Type[iB]& 0xf0) != 0xc0) && ((cSpyB_Type[iB]& 0xf0) != 0xd0) && ((cSpyB_Type[iB]& 0xf0) != 0xe0) && ((cSpyB_Type[iB]& 0xf0) != 0xf0)){
		overflowB++;
		//	cout << "2.5" << endl;
		rocheaderB = false;
		colAddrB = 0;
		...
		if ((cSpyB_Type[iB]& 0xf0) == 0x70){
		  rocheaderB = true;
		  overflowB = 0;
		  //rocB++;                                                                  // count roc header till we see (part of) tbm header or trailer
		  while (((iB+1) < sizeB) && (rocheaderB == true)  && ((cSpyB_Type[iB]& 0xf0) != 0x80) && ((cSpyB_Type[iB]& 0xf0) != 0x90) && ((cSpyB_Type[iB]& 0xf0) != 0xa0) &&  // till we see (part of) tbm header or trailer
		                     	     ((cSpyB_Type[iB]& 0xf0) != 0xb0) && ((cSpyB_Type[iB]& 0xf0) != 0xc0) && ((cSpyB_Type[iB]& 0xf0) != 0xd0) && ((cSpyB_Type[iB]& 0xf0) != 0xe0) && ((cSpyB_Type[iB]& 0xf0) != 0xf0) ){
		   if ((cSpyB_Type[iB]& 0xf0) == 0x70){
		     rocB++;
		     //	     cout << "rocB: " << rocB << endl;
		   }
		    if (((iB+6) < sizeB)  && ((cSpyB_Type[iB+1]& 0xf0) == 0x10) && ((cSpyB_Type[iB+2]& 0xf0) == 0x20) && ((cSpyB_Type[iB+3]& 0xf0) == 0x30) && 
		                                                   ((cSpyB_Type[iB+4]& 0xf0) == 0x40) && ((cSpyB_Type[iB+5]& 0xf0) == 0x50) && ((cSpyB_Type[iB+6]& 0xf0) == 0x60) ) {
		      //cout << Col Addr << Row Addr << Pixel Hit << endl;
		      ...
		      word = ((cSpyB_Type[iB+1] & 0xf) << 20) + ((cSpyB_Type[iB+2]& 0xf) << 16) + ((cSpyB_Type[iB+3]& 0xf) << 12) + ((cSpyB_Type[iB+4]& 0xf) << 8) + ((cSpyB_Type[iB+5]& 0xf) << 4) + (cSpyB_Type[iB+6]& 0xf);

		      pixelHitB = (word & 0x0f) + ((word >> 1) & 0xf0);
		      ...
		      RocNrB.push_back(rocB);
		      ColAddrB.push_back(colAddrB);
		      RowAddrB.push_back(rowAddrB);
		      PixelHitB.push_back(pixelHitB);
		      // cout << dec << "Hit: " << rocB << "\t" << colAddrB << "\t" << rowAddrB << "\t" << pixelHitB << endl;
		      colAddrB = 0;
		      rowAddrB = 0;
		      pixelHitB = 0;
		  ...
		  if (overflowB >=20){      // has to be somewhere else, but not here
		    overflowB = 0;
		    notrailerB++;
		    rocheaderB = false;
		    cout << "tbmB header but no trailer in sight" << endl;
		...
		if (((cSpyB_Type[iB]& 0xf0) == 0x70) || ((cSpyB_Type[iB]& 0xf0) == 0x80) || ((cSpyB_Type[iB]& 0xf0) == 0x90) || ((cSpyB_Type[iB]& 0xf0) == 0xa0) ||  // till we see (part of) tbm header or trailer
		    ((cSpyB_Type[iB]& 0xf0) == 0xb0) || ((cSpyB_Type[iB]& 0xf0) == 0xc0) || ((cSpyB_Type[iB]& 0xf0) == 0xd0) || ((cSpyB_Type[iB]& 0xf0) == 0xe0) || ((cSpyB_Type[iB]& 0xf0) == 0xf0) ){
		  iB--;	}

		iB++;  // gives us a problem, because if header/trailer is found it arises nevertheless by one
	      ...
	      if (rocB == 8){
		goodwordB++;
		for (int i = 0; i < ColAddrB.size(); i++){
		  cout << dec << (RocNrB[i] + 8) << "\t" << ColAddrB[i] << "\t" << RowAddrB[i] << "\t" << PixelHitB[i] << endl;}                         // (RocNrB[i] + 6) so that the rocs in A are 1-6 and rocs in B 7-12
		RocNrB.clear();
		ColAddrB.clear();
		RowAddrB.clear();
		PixelHitB.clear();
		...
		cout << "not good" << endl;
		//if (iB < (sizeB + 2)){
		//  cout << "errorNrB: " << rocerrorB << "  " << cSpyB_Type[iB-10] << "  " << cSpyB_Type[iB-9] << "  " << cSpyB_Type[iB-8] << "  " << cSpyB_Type[iB-7] << "  " << cSpyB_Type[iB-6] << "  " << cSpyB_Type[iB-5] << "  " 
		//       << cSpyB_Type[iB-4] << "  " << cSpyB_Type[iB-3] << "  " << cSpyB_Type[iB-2] << "  " << cSpyB_Type[iB-1] << "  " << cSpyB_Type[iB] << "  " << cSpyB_Type[iB+1] << "  " << cSpyB_Type[iB+2] << endl;}
		//else{ 
		//  cout << "errorNrB: " << rocerrorB << "  " << cSpyB_Type[iB-10] << "  " << cSpyB_Type[iB-9] << "  " << cSpyB_Type[iB-8] << "  " << cSpyB_Type[iB-7] << "  " << cSpyB_Type[iB-6] << "  " << cSpyB_Type[iB-5] << "  " 
		//       << cSpyB_Type[iB-4] << "  " << cSpyB_Type[iB-3] << "  " << cSpyB_Type[iB-2] << "  " << cSpyB_Type[iB-1] << "  " << cSpyB_Type[iB] << endl;}
		...
		rocB = 0;}
	      ...
	      if ( (iB < (sizeB + 4)) && ((cSpyB_Type[iB]& 0xf0) == 0xc0) && ((cSpyB_Type[iB+1]& 0xf0) == 0xd0) && ((cSpyB_Type[iB+2]& 0xf0) == 0xe0) && ((cSpyB_Type[iB+3]& 0xf0) == 0xf0) )     // if after ROC header tbm trailer: skip this tbm header
		iB += 4;
	    }
...
void exec(SimpleCommand* c){
  ...
  if(c->Keyword("fed", slot)){
   ...
   BgoTrigReadspyFifo();
  ...
  }else if(c->Keyword("confcal")) {
    ConfCal();
  }else if(c->Keyword("titlecalinject", n)) {
    TitleCalInject(n);
  }
    ...
    cout << "I'm sure you mean something else than  <" << *c << ">" << endl;
  }
