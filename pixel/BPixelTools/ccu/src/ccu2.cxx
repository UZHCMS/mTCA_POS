 ...
 //************************************power
  ...
  //************************************
	...
	return result;
      ...
      unsigned int AddressChannel = strtol (tokens[1].c_str(),NULL,16);      // the second input word is channel
      deviceAddress = strtol (tokens[2].c_str(),NULL,16);                    // the third input word is address
      unsigned int value = strtol (tokens[3].c_str(),NULL,16);               // the fourth input word is value
      ...
      if (tokens[1]=="poh1"){
	tokens[3]=tokens[2];
	tokens[1]="CHANNEL FOR POH1";
        tokens[2]="ADDRESS FOR POH1";
	  }
      
      else if (tokens[1]=="poh2"){
	tokens[3]=tokens[2];
	tokens[1]="CHANNEL FOR POH2";
        tokens[2]="ADDRESS FOR POH2";
	  }

      else if (tokens[1]=="poh3"){
	tokens[3]=tokens[2];
	tokens[1]="CHANNEL FOR POH3";
        tokens[2]="ADDRESS FOR POH3";
	  }
      
      else if (tokens[1]=="poh4"){
	tokens[3]=tokens[2];
	tokens[1]="CHANNEL FOR POH4";
        tokens[2]="ADDRESS FOR POH4";
	  }

      else if (tokens[1]=="poh5"){
	tokens[3]=tokens[2];
	tokens[1]="CHANNEL FOR POH5";
        tokens[2]="ADDRESS FOR POH5";
	  }
      
      else if (tokens[1]=="poh6"){
	tokens[3]=tokens[2];
	tokens[1]="CHANNEL FOR POH6";
        tokens[2]="ADDRESS FOR POH6";
	  }

      else if (tokens[1]=="poh7"){
	tokens[3]=tokens[2];
	tokens[1]="CHANNEL FOR POH7";
	tokens[2]="ADDRESS FOR POH7";
	  }

      else if (tokens[1]=="poh8"){
	tokens[3]=tokens[2];
	tokens[1]="CHANNEL FOR POH8";
        tokens[2]="ADDRESS FOR POH8";
	  }
      
      else if (tokens[1]=="poh9"){
	tokens[3]=tokens[2];
	tokens[1]="CHANNEL FOR POH9";
        tokens[2]="ADDRESS FOR POH9";
	  }

      else if (tokens[1]=="poh10"){
	tokens[3]=tokens[2];
	tokens[1]="CHANNEL FOR POH10";
        tokens[2]="ADDRESS FOR POH10";
	  }
      
      else if (tokens[1]=="poh11"){
	tokens[3]=tokens[2];
	tokens[1]="CHANNEL FOR POH11";
        tokens[2]="ADDRESS FOR POH11";
	  }

      else if (tokens[1]=="poh12"){
	tokens[3]=tokens[2];
	tokens[1]="CHANNEL FOR POH12";
        tokens[2]="ADDRESS FOR POH12";
	  }
      
      else if (tokens[1]=="poh13"){
	tokens[3]=tokens[2];
	tokens[1]="CHANNEL FOR POH13";
        tokens[2]="ADDRESS FOR POH13";
	  }

      else if (tokens[1]=="poh14"){
	tokens[3]=tokens[2];
	tokens[1]="CHANNEL FOR POH14";
        tokens[2]="ADDRESS FOR POH14";
	  }

      else { 
	return sting("wrong syntax")
	  }
  ...
  //************************************POHs
	...
	return string("wrong number of parameters! poh setall needs 8 parameters");
      ...
      return setPoh ( fecAccess, fecAddress, ringAddress, ccuAddress, channelAddress, deviceAddress, loop, tms, true, init_poh1_gain0, true, init_poh1_gain1, true, init_poh1_gain2, true, init_poh1_gain3, true, init_poh1_bias0, true, init_poh1_bias1, true, init_poh1_bias2, true, init_poh1_bias3);
    }
    else return string("not an option! options for poh set: [g0],[g1],[g2],[g3],[b0],[b1],[b2],[b3]"); 
  }  
  //************************************DEVICE
